...
class Generator(nn.Module):
	...
	def __init__(self, args, device):
		...
		self.main = nn.Sequential(
			## Input: latent vector z -> convolution
			nn.ConvTranspose2d(self.args.z_dim + self.args.c_dim, self.args.input_dim * 8, 4, 2, 0),
			nn.BatchNorm2d(self.args.input_dim * 8),
			nn.ReLU(True),

			## State size: (self.args.input_dim * 8) x 4 x 4
			nn.ConvTranspose2d(self.args.input_dim * 8, self.args.input_dim * 4, 4, 2, 1),
			nn.BatchNorm2d(self.args.input_dim * 4),
			nn.ReLU(True),
			
			## State size: (self.args.input_dim * 4) x 8 x 8
			nn.ConvTranspose2d(self.args.input_dim * 4, self.args.input_dim * 2, 4, 2, 1),
			nn.BatchNorm2d(self.args.input_dim * 2),
			nn.ReLU(True),
			
			## State size: (self.args.input_dim * 2) x 16 x 16
			nn.ConvTranspose2d(self.args.input_dim * 2, self.args.input_dim, 4, 2, 1),
			nn.BatchNorm2d(self.args.input_dim),
			nn.ReLU(True),
			
			## State size: (self.args.input_dim) x 32 x 32
			nn.ConvTranspose2d(self.args.input_dim, self.args.n_channel, 4, 2, 1),
			nn.Tanh()
			## State size: (self.args.n_channel) x 64 x 64
		)
...
class Discriminator(nn.Module):
	...
	def __init__(self, args, device):
		...
		self.main = nn.Sequential(
			## Input size: (self.args.n_channel + 1) x 64 x 64
			nn.Conv2d(self.args.n_channel + 1, self.args.input_dim, 4, 2, 1),
			nn.LeakyReLU(inplace=True),#nn.LeakyReLU(0.2, inplace=True),

			## State size: (self.args.input_dim) x 32 x 32
			nn.Conv2d(self.args.input_dim, self.args.input_dim * 2, 4, 2, 1),
			nn.BatchNorm2d(self.args.input_dim * 2),
			nn.LeakyReLU(inplace=True),#nn.LeakyReLU(0.2, inplace=True),
			
			## State size: (self.args.input_dim * 2) x 16 x 16
			nn.Conv2d(self.args.input_dim * 2, self.args.input_dim * 4, 4, 2, 1),
			nn.BatchNorm2d(self.args.input_dim * 4),
			nn.LeakyReLU(inplace=True),#nn.LeakyReLU(0.2, inplace=True),
			
			## State size: (self.args.input_dim * 4) x 8 x 8
			nn.Conv2d(self.args.input_dim * 4, self.args.input_dim * 8, 4, 2, 1),
			nn.BatchNorm2d(self.args.input_dim * 8),
			nn.LeakyReLU(inplace=True),#nn.LeakyReLU(0.2, inplace=True), 

			## State size: (self.args.input_dim * 8) x 4 x 4
			nn.Conv2d(self.args.input_dim * 8, 1, 4, 1, 0), 
			nn.Sigmoid()
		)
