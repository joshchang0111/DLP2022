...
#include "progressbar.hpp" // NEW: progress bar from https://github.com/gipert/progressbar
...
int main(int argc, const char* argv[]) {
	...
	/**
	if (experiment.find("lr") != std::string::npos || experiment.find("ori") != std::string::npos)
		info << "Use original tuple\t";
	if (experiment.find("tuple1") != std::string::npos)
		info << "Use tuple1\t";
	if (experiment.find("tuple2") != std::string::npos)
		info << "Use tuple2\t";
	info << std::endl;
	return 0;
	*/
	...
	float alpha = 0.1; //0.05; //0.0025; //0.01;
